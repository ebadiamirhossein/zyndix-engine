...
**File:** `06-build-progress.md` · **Started:** 2026-07-08 · **Last reconciled:** 2026-09-25 (Session 11)
...
| DNS — `getzyndix.com` (Microsoft 365) | ✅ **DMARC present** · 🟨 mail-tester re-test open | U5 | `dig` 2026-09-25: MX `getzyndix-com.mail.protection.outlook.com`, SPF `include:spf.protection.outlook.com -all`, DKIM `selector1`/`selector2._domainkey` CNAMEs resolve to M365 keys. **DMARC added by the operator via dmarcly; Session 11 `dig` shows exactly ONE `_dmarc` TXT** on 1.1.1.1, 8.8.8.8 and both authoritative NS (`dns1`/`dns2.registrar-servers.com`): `v=DMARC1; p=none; rua=mailto:…@ag.dmarcly.com; ruf=mailto:…@fo.dmarcly.com; sp=none;` — no CNAME at `_dmarc`. The mail-tester re-test of both getzyndix mailboxes is still to do |
| Tracking CNAME (`track.`) | ✅ **not needed — closed** | — | Operator decision 2026-09-25 (§5): **no custom tracking domain**. Workspace "Disable Open Tracking" ON and "Always send first email as text-only" ON (operator-reported — the workspace API exposes neither), link tracking off. Enforced per campaign and read back live by `instantly-sender-campaigns.ts --verify` (Session 11). `dig`: no `track` CNAME on either domain, as intended |
| **Instantly Growth + 4 mailboxes** | ✅ **live, warming** · 🟨 daily_limit 4 ≠ intended 1 | U4 | Bought 2026-09-24 (**Growth**, not Hypergrowth — §5). Live read 2026-09-25 (`live-instantly.ts --all`): workspace `Zyndix`, `plan_id pid_g_v2`, **4 accounts, all status active / warmup active / setup_pending false / warmup score 100**. Warmup started 2026-09-24 19:31–20:52 UTC (all four). **Session 11 read (`GET /api/v2/accounts`): `daily_limit=4` on all four** — the operator set it to **1** as a pre-first-send safety net, so the saved value differs (§6). `sending_gap=1`; `enable_slow_ramp` **true** on the zyndixhq accounts, **false** on the getzyndix ones. **4 campaigns** since Session 11 — one draft campaign per mailbox (sender pinning, below) |
...
| `INSTANTLY_API_KEY` | ✅ **verified** | U4 | Present in `.env.local`. Read scopes proven live 2026-09-25 (workspaces, accounts, campaigns, webhook event types). **`campaigns:create` proven live in Session 11** (4 draft campaigns created). Still **not exercised**: `leads:create`, `leads:delete`, `campaigns:update`, `emails:create` (threaded replies), `emails:read`, `block_list_entries:create` — first used in U6 |
| Mailboxes pass mail-tester ≥9/10 | ✅ score gate met · 🟨 getzyndix auth open | U5 | All four scored **9.6/10** (operator-reported). `zyndixhq.com` mailboxes: "properly authenticated". **`getzyndix.com` mailboxes: "You're not fully authenticated"** — DKIM had been enabled minutes before the test and DMARC was absent; **DMARC now exists (Session 11)**. **Re-test pending; not passed** |
...
| 2026-09-25 | **`getzyndix.com` "You're not fully authenticated" on mail-tester**, despite 9.6/10 on both mailboxes. DKIM had been enabled minutes before the test. `dig`: both DKIM selectors resolve, but `_dmarc.getzyndix.com` was NXDOMAIN | 🟨 **DMARC fixed; re-test open** — operator | Session 11: exactly one `_dmarc` TXT (dmarcly, `p=none`) on public and authoritative resolvers. Still to do: re-run mail-tester on both getzyndix mailboxes. Not passed until the re-test says authenticated |
| 2026-09-25 | **Instantly account `daily_limit` is 4, not the intended 1.** Session 10 read 30; the operator then set the daily campaign limit to 1 on all four as a pre-first-send safety net; the Session 11 API read shows **4** on every account | 🟨 open — operator | Check the value in the Instantly UI (the field may round, or a different setting was changed). Changing it is a live-settings write, so the engine did not touch it. With draft-only campaigns and no leads nothing can send, and the engine ledger (15→30 ramp) caps sends regardless. Re-read with `live-instantly.ts --accounts` |
| 2026-09-25 | **`enable_slow_ramp` differs by domain**: true on the zyndixhq accounts, false on the getzyndix ones (Session 11 read) | 🟨 open — operator | Probably a default of each connection path. Harmless while nothing sends; make it consistent before U6 if intended |
| 2026-09-25 | **`link_tracking` is not echoed by Instantly.** The sender campaigns were created with `link_tracking:false`, but `GET /api/v2/campaigns/{id}` omits the field (as it does `stop_on_auto_reply:false`), while the required `open_tracking:false` is echoed | 🟨 open — low | Inferred as "false-valued optional flags are omitted". `--verify` flags drift only if it is ever `true`, and prints "not echoed". Confirm once in the Instantly UI for one campaign |
...
| 2026-09-25 | **Every current lead would be held `timezone_unknown`.** All 34 companies are `country = 'US'` with null timezones, and all 34 leads have null timezones; the US has many zones, so no country fallback applies | 🟨 open — operator/U15 | Correct behaviour under the operator's rule, not a bug. Before the first US prospect send, set `leads.timezone`, for example from Apollo's person or organization location data (U15 evidence model) |
| 2026-09-25 | **The verify stage's `isSuppressed` treats any domain match as suppressed** — including the `domain` it writes beside an invalid email, so one invalid address suppresses its colleagues at the source stage | 🟨 open — backlog (`09` §5) | Out of U5's scope (verify stage). U5's own pre-send check uses person-level versus company-wide scope. Worth aligning in U6, which owns suppression |
...
| send_policy | v1 | 2026-09-25 | **New (U5, Session 11), operator-approved seed.** `verification_max_age_days 90`, `allow_catch_all false`, `min_warmup_score 80`, `duplicate_company_window_days 30`. Read by the send stage's preflight |
...
`linkedin_senders` is agreed but not yet created. `operations_pause` arrives at U9. 28 settings rows across 13 keys, confirmed by `ping.ts` 2026-09-21; `send_policy` v1 (2026-09-25) makes 29 rows across 14 keys.
