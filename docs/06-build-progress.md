...
**File:** `06-build-progress.md` · **Started:** 2026-07-08 · **Last reconciled:** 2026-09-25 (Session 12)
...
| U6 | Webhooks, reply freeze, suppression 🚩 | 🟨 **in progress (1 of 3 sessions)** · Part 1 core **tested locally** · webhook creation **verified with provider** | Session 12: `/api/webhooks/instantly` + `lib/webhooks/instantly.ts` (token auth, persist-first, hash dedupe, reply freeze without any model call, auto-reply recorded only, unsubscribe and bounce suppression, bounce-rate auto-pause, exceptions queue). `pnpm test:webhooks` → **58/58** against Supabase (mocked Instantly, fetch guard proving zero network calls); `pnpm test:webhook-rules` → **10/10**. Live on Growth: a webhook was created, Instantly's test delivery arrived through a Cloudflare quick tunnel, was stored and processed, then the webhook was deleted. **Still to do (Sessions 13–14): reconcile job (`stop_processing_stale`, missed-reply polling), sourced→sent traversal plus 8 stop-rule siblings, and the Part 2 live drill.** 🚩 not yet reached |
...
| 2026-09-25 | **Two legacy Instantly webhooks** from 2025-10-15 (`email_sent`, `reply_received` → `hook.eu1.make.com`, no auth header), from the operator's old Make.com campaign. Once sending starts, reply text would also reach that scenario | 🟨 open — operator, **gate before the live drill** | Left untouched on the operator's instruction. They must be deleted or confirmed before the U6 live drill; ask again at that gate |
...
| 2026-09-25 | **The 4 existing drafts still carry the v2 footer ("— Amir")**, and Steffen's approved touch was hashed under the U5 snapshot | 🟨 open — operator | By design: Steffen's touch refuses `stale_approval`; all 4 bodies refuse approval as "signs itself" until edited (remove the "— Amir" line) or redrafted. Nothing can send meanwhile |
